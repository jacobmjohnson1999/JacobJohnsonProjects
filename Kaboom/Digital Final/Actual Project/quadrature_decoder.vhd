...
ENTITY quadrature_decoder IS
	GENERIC(
		positions					:	INTEGER := 570; 		--size of the position counter (i.e. number of positions counted)
		debounce_time				:	INTEGER := 25_000; 		--number of clock cycles required to register a new position = debounce_time + 2
			...
			ELSIF(debounce_cnt = debounce_time	--debounce time for a and b is met
				...
				IF((b_prev XOR a_new(1)) = '1') THEN	--clockwise direction
					IF(position < positions-1) THEN		--not at position limit
						position <= position + 4;		--advance position counter
					END IF;
				ELSE								--counter-clockwise direction
					IF(position > 0) THEN			--not at position limit
						position <= position - 4;	--decrement position counter
					END IF;
				END IF;
			END IF;
		END IF;
